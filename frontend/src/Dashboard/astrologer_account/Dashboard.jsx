...
import JyotishAbout from "../../pages/Jyotish/JyotishAbout";
import Profile from "./Profile";
import Appointments from "./Appointments";

const Dashboard = () => {
  const { data, loading, error } = useGetProfile(
    `${BASE_URL}/astrologers/profile/me`
  );
              ...
              {data.isApproved === "pending" && (
                <div className="flex p-6 mb-4 text-yellow-800 bg-yellow-50 rounded-lg">
                  ...
                  <div className="ml-3 text-sm font-medium">
                    We will review and approve within 3 days.
                  </div>
                </div>
              )}
              <div className="mt-2">
                ...
                {tab === "settings" && <Profile astrologersData={data}/>}
              </div>
            </div>
          </div>
        )}
