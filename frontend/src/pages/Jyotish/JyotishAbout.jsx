...
const JyotishAbout = ({ name, about, qualifications, experiences }) => {
  return (
    <div className=" p-8 rounded-xl shadow-xl max-w-4xl mx-auto">
      <div>
        <h3 className="text-3xl font-bold text-pruple-100 flex items-center gap-3">
          About{" "}
        ...
        <div className="mt-12 border-t pt-8 border-gray-300">
          ...
          <ul className="mt-6 space-y-8">
            {qualifications?.map((item, index) => (
              <li
                key={index}
                className="flex flex-col sm:flex-row sm:justify-between sm:items-center md:gap-6 p-4 bg-white/90 rounded-lg shadow-md border transition-all duration-300 ease-in-out"
              >
                <div>
                  <span className="text-indigo-800 text-sm font-semibold bg-white py-1 px-4 rounded-full inline-block mb-3">
                    {FormatDate(item.startingDate)} - {FormatDate(item.endingDate)}
                  </span>
                  <p className="text-lg font-medium text-gray-800">{item.degree}</p>
                </div>
                <p className="text-sm font-medium text-black mt-2 sm:mt-0 bg-blue-100 py-2 px-4 rounded-md">
                  {item.university}
                </p>
              </li>
            ))}
